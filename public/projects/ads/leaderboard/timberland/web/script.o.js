...
(function($, window, undefined){
  ...
  function init()
  {
    ...
    TweenMax.from('.logo', 1, {opacity:0, x:100, ease:Back.easeIn});
    TweenMax.to('.expanded .landing .col img', 0, {scaleX:_scale, scaleY:_scale});
    
    // auto expand
    thisExpand(); $(".ad").fadeToggle();
  }
  ...
  function setEvents()
  {
    ...
    $('.expanded .carousel .btnShopnow').hover(
      function(){
        TweenMax.to($('.expanded .carousel .products-container .products:eq('+ (_itemID) +') .circle'), 0.25, {scaleX:0.96, scaleY:0.96, rotation:-360});
      },
      function(){
        TweenMax.to($('.expanded .carousel .products-container .products:eq('+ (_itemID) +') .circle'), 0.25, {scaleX:1, scaleY:1, rotation:0});
      }
    );
    
    $('.expanded .landing .col').hover(
      function(){
        TweenMax.to($(this).find('img'), 0.5, {scaleX:1, scaleY:1, opacity:1});
        $('.expanded .buttons a').eq($(this).index()).addClass('active');
        $(this).find('.box').toggleClass('orange');
      },
      function(){
        TweenMax.to($(this).find('img'), 0.5, {scaleX:_scale, scaleY:_scale, opacity:0.5});
        if(!_landingHide){
          $('.expanded .buttons a').removeClass('active');
        }
        $(this).find('.box').toggleClass('orange');
    ...
    if(_productID > 0){
      ...
      $('.expanded .videoholder .video').empty().html(
        ...
        '<object id="myExperience" class="BrightcoveExperience">'+
          ...
          '<param name="playerID" value="3300779005001" />'+
          '<param name="playerKey" value="AQ~~,AAAAnrRf0Lk~,7eKhRjThoOPWTQqeccEi5xCWTNni_6e4" />'+
          ...
          '<param name="@videoPlayer" value="3248962290001" />'+
        '</object>'+
        '<script type="text/javascript">brightcove.createExperiences();</script>'
      );
    }
    
    TweenMax.to('.expanded .vc-holder .vc-contents', init ? 0 : 0.5, {y:-(474 * _productID)});
  }
