...
(function($, window, undefined){
	...
	function setBtnClose(e)
	{
		$('#btnClose').show().hover(
			function(){
				$(this).attr('src', ADTECH.getFileUrl('btn_close_over.png'));
			},
			function(){
				$(this).attr('src', ADTECH.getFileUrl('btn_close.png'));
			}
		).on('click', function(){
			$(this).hide();
	...
	{
		$('.footer .fcarousel').append('<div id="carouselThumbs"></div><div id="carouselThumbsArrows"><img src="'+ ADTECH.getFileUrl('btn_arrow_left.png') +'" class="arrowLeft" /><img src="'+ ADTECH.getFileUrl('btn_arrow_right.png') +'" class="arrowRight" /></div>');
		$.each(carouselData.data, function(index, obj){
			var index = index + 1;
			$('#carouselThumbs').append('<img src="'+ ADTECH.getFileUrl('carousel_thumb_'+ index +'.jpg') +'" data-id="'+ index +'" />');
		});
		...
		$('#carouselThumbsArrows .arrowLeft').hover(
			function(){
				$(this).attr('src', ADTECH.getFileUrl('btn_arrow_left_over.png'));
			},
			function(){
				$(this).attr('src', ADTECH.getFileUrl('btn_arrow_left.png'));
			}
		).on('click', function(){
		...
		$('#carouselThumbsArrows .arrowRight').hover(
			function(){
				$(this).attr('src', ADTECH.getFileUrl('btn_arrow_right_over.png'));
			},
			function(){
				$(this).attr('src', ADTECH.getFileUrl('btn_arrow_right.png'));
			}
		).on('click', function(){
			...
			$('.contents').append(
				'<div id="carouselMainContainer">'+
					...
					'<div id="carouselMainArrows">'+
						'<img src="'+ ADTECH.getFileUrl('btn_arrow_left.png') +'" class="arrowLeft" />'+
						'<img src="'+ ADTECH.getFileUrl('btn_arrow_right.png') +'" class="arrowRight" />'+
					'</div>'+
				'</div>'
			);
			$.each(carouselData.data, function(index, obj){
				...
				html += '<div class="carouselImage">';
					html += '<img src="'+ ADTECH.getFileUrl('carousel_main_'+ index +'.jpg') +'" class="mainImage" />';
					html += '<div class="carouselText">';
						html += '<div class="title"><em>'+ obj.name +'</em></div>';
						html += '<div>'+ obj.desc +'</div>';
						html += '<div><a href="#"><span>SHOP NOW<img src="'+ ADTECH.getFileUrl('arrow_shopnow.png') +'" id="arrowShopNow" /></span></a></div>';
					html += '</div>';
					$.each(obj.info, function(iCnt, iObj){
						...
						html += '<div class="carouselInfo" style="top:'+ iObj.ybtn +'px; '+ xbox +'px;">';
							html += '<div class="carouselInfoHolder">';
								...
								html += '<div class="infoCopy '+ infoCopy +'">'+ iObj.copy +'<p><a href="#"><span>SHOP NOW<img src="'+ ADTECH.getFileUrl('arrow_shopnow.png') +'" id="arrowShopNow" /></span></a></p></div>';
								html += '<img src="'+ ADTECH.getFileUrl('btn_plus.png') +'" class="infoBtn '+ infoBtn +'" />';
							html += '</div>';
						html += '</div>';
					});
				html += '</div>';
	...
	function setBtnRoundel(show)
	{
		if(show) 
		{
			$('#holderContents').append('<div id="btnRoundel"><a href="http://www.selfridges.com" target="_blank"><img src="'+ ADTECH.getFileUrl('btn_roundel.png') +'"></a></div>');
		} else {
			$('#btnRoundel').remove();
		}
	}
