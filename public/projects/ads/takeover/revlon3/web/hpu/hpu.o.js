...
$(document).ready(function(){
    ...
    if(supportsLocalStorage())
    {
    	...
    	$('.carousel .arrow.left, .carousel .arrow.right').click(function(){
			if(!_this.carousel.animate){
				_this.carousel.animate = true;
    ...
    function trackEventProduct(thisID)
    {
    	switch(thisID){
			case 2:
				ADTECH.event('HPU display colour emerald green');
				break;
			case 3:
				ADTECH.event('HPU display colour raspberry');
				break;
			case 4:
				ADTECH.event('HPU display colour red');
				break;
			case 5:
				ADTECH.event('HPU display colour velvet');
				break;
			default:
	...
	{		
		...
		$('.carousel ul').animate({'left':'-'+ (carouselWidth * _this.carousel.id) +'px'}, 500, function(){
			clearInterval(_thisIntervalID);
			setIntervalChange();
			...
			$('.carousel ul').css({'left':'-'+ (carouselWidth * _this.carousel.id)+ 'px'});
			
			_this.carousel.animate = false;
		});
