...
(function($, window, undefined){
	...
	{
		_timeline.add(TweenMax.to('.box2', _normDuration, {css:{'width':970}}));
		_timeline.add(TweenMax.from('.polaroids', _normDuration, {css:{'bottom':-300, rotation:-45}}));
		_timeline.add(TweenMax.from('.copy', _normDuration, {css:{'right':-300}, ease:Bounce.easeOut}));
	}
	...
	function timelinePlay()
	{
		...
		TweenMax.delayedCall(0.5, function(){
			TweenMax.to('.photographers', 4, {css:{'left':_itemID == 1 ? 0 : -304}, ease:Linear.easeNone, onComplete:function(){
				_timeline.play();
				photographersHide();
			}});
		});
